...
import com.abhinav.covid19.vaccinetracker.kafka.KafkaStateStores;
...
import com.abhinav.covid19.vaccinetracker.utils.Utils;
...
import static com.abhinav.covid19.vaccinetracker.utils.Utils.translateName;
                    ...
                    if (ctx.update().hasMessage() && ctx.update().getMessage().hasText()) {
                        ...
                        if (Utils.isValidPincode(pincode)) {
                            ...
                            String channelMsg = String.format("User %s (%s) requested stats for %s via text %s",
                                    translateName(ctx.update().getMessage().getChat()), chatId, "Yesterday", pincode);
